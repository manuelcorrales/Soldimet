...
export class CajaService {
  ...
  protected convertDateFromClient(caja: ICaja): ICaja {
    return Object.assign({}, caja, {
      fecha: caja.fecha?.isValid() ? caja.fecha.format(DATE_FORMAT) : undefined,
      horaApertura: caja.horaApertura?.isValid() ? caja.horaApertura.toJSON() : undefined,
      horaCierre: caja.horaCierre?.isValid() ? caja.horaCierre.toJSON() : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.horaApertura = res.body.horaApertura ? dayjs(res.body.horaApertura) : undefined;
      res.body.horaCierre = res.body.horaCierre ? dayjs(res.body.horaCierre) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((caja: ICaja) => {
        caja.fecha = caja.fecha ? dayjs(caja.fecha) : undefined;
        caja.horaApertura = caja.horaApertura ? dayjs(caja.horaApertura) : undefined;
        caja.horaCierre = caja.horaCierre ? dayjs(caja.horaCierre) : undefined;
      });
    }
    return res;
  }
