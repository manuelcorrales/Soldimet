...
export class PagoChequeDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.bancoService.query()
            .subscribe((res: ResponseWrapper) => { this.bancos = res.json; }, (res: ResponseWrapper) => this.onError(res.json));
        this.formaDePagoService
            ...
            .subscribe((res: ResponseWrapper) => {
                if (!this.pagoCheque.formaDePago || !this.pagoCheque.formaDePago.id) {
                    ...
                    this.formaDePagoService
                        .find(this.pagoCheque.formaDePago.id)
    ...
    save() {
        ...
        if (this.pagoCheque.id !== undefined) {
            this.subscribeToSaveResponse(
                this.pagoChequeService.update(this.pagoCheque));
        } else {
            this.subscribeToSaveResponse(
                this.pagoChequeService.create(this.pagoCheque));
        }
    }

    private subscribeToSaveResponse(result: Observable<PagoCheque>) {
        result.subscribe((res: PagoCheque) =>
        ...
        this.routeSub = this.route.params.subscribe((params) => {
            if ( params['id'] ) {
                this.pagoChequePopupService
                    .open(PagoChequeDialogComponent as Component, params['id']);
            } else {
                this.pagoChequePopupService
                    .open(PagoChequeDialogComponent as Component);
            }
        });
    }
