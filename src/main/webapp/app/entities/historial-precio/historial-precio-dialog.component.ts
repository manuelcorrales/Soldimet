        ...
        this.precioRepuestoService
            ...
            .subscribe((res: ResponseWrapper) => {
                if (!this.historialPrecio.precioRepuesto || !this.historialPrecio.precioRepuesto.id) {
                    ...
                    this.precioRepuestoService
                        .find(this.historialPrecio.precioRepuesto.id)
                        .subscribe((subRes: PrecioRepuesto) => {
    ...
    save() {
        ...
        if (this.historialPrecio.id !== undefined) {
            this.subscribeToSaveResponse(
                this.historialPrecioService.update(this.historialPrecio));
        } else {
            this.subscribeToSaveResponse(
                this.historialPrecioService.create(this.historialPrecio));
        }
    }

    private subscribeToSaveResponse(result: Observable<HistorialPrecio>) {
        result.subscribe((res: HistorialPrecio) =>
        ...
        this.routeSub = this.route.params.subscribe((params) => {
            if ( params['id'] ) {
                this.historialPrecioPopupService
                    .open(HistorialPrecioDialogComponent as Component, params['id']);
            } else {
                this.historialPrecioPopupService
                    .open(HistorialPrecioDialogComponent as Component);
            }
        });
    }
