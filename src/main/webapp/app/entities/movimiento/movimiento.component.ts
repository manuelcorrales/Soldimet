...
export class MovimientoComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.movimientoService
      ...
      .subscribe(
        (res: HttpResponse<IMovimiento[]>) => this.paginateMovimientos(res.body, res.headers),
  ...
  trackId(index: number, item: IMovimiento) {
    return item.id;
  }

  registerChangeInMovimientos() {
    this.eventSubscriber = this.eventManager.subscribe('movimientoListModification', response => this.loadAll());
  }
  ...
  protected paginateMovimientos(data: IMovimiento[], headers: HttpHeaders) {
